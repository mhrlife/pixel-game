...
func (rq *RoomQuery) QueryAdmins() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, room.AdminsTable, room.AdminsPrimaryKey...),
		)
...
func (rq *RoomQuery) WithAdmins(opts ...func(*UserQuery)) *RoomQuery {
	query := (&UserClient{config: rq.config}).Query()
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withAdmins; query != nil {
		if err := rq.loadAdmins(ctx, query, nodes,
			func(n *Room) { n.Edges.Admins = []*User{} },
			func(n *Room, e *User) { n.Edges.Admins = append(n.Edges.Admins, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoomQuery) loadAdmins(ctx context.Context, query *UserQuery, nodes []*Room, init func(*Room), assign func(*Room, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Room)
	nids := make(map[int64]map[*Room]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(room.AdminsTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(room.AdminsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(room.AdminsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(room.AdminsPrimaryKey[0]))
