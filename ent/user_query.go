...
// QueryPixels chains the current query on the "pixels" edge.
func (uq *UserQuery) QueryPixels() *PixelQuery {
	query := (&PixelClient{config: uq.config}).Query()
...
// WithPixels tells the query-builder to eager-load the nodes that are connected to
// the "pixels" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithPixels(opts ...func(*PixelQuery)) *UserQuery {
	query := (&PixelClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPixels; query != nil {
		if err := uq.loadPixels(ctx, query, nodes,
			func(n *User) { n.Edges.Pixels = []*Pixel{} },
			func(n *User, e *Pixel) { n.Edges.Pixels = append(n.Edges.Pixels, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
